...
function getlist($today = false) {
  ...
  if ($today) {
    ...
    $sql = "select a.*, c.first_name as doctor, b.name, b.phone, b.address from pet_test_usg a inner join pet_users c on a.userid = c.userid inner join pet_test_customer b on a.customerid = b.id where (b.name like '%$key%' or b.phone like '%$key%') and status < 5 order by a.calltime desc, a.recall desc limit 50";
    $list = dataCover($db->all($sql));
  }
...
function uncalled() {
  ...
  $sql = "update pet_test_usg set status = 1, note = '". $data->note ."', called = $time, recall = $recall where id = $data->id";
  $db->query($sql);
  $result['status'] = 1;
...
function dead() {
  ...
  $sql = "update pet_test_usg set status = 4, note = '$data->note' where id = $data->id";
  $db->query($sql);
  $result['status'] = 1;
...
function insert() {
  ...
  $sql = "insert into pet_test_usg (customerid, userid, cometime, calltime, recall, number, status, note, time, called) values ($customerid, $userid, $data->cometime, $data->calltime, $data->calltime, $data->number, 0, '$data->note', ". time() .", 0)";
  $result['status'] = 1;
...
//   $sql = "select a.*, c.first_name as doctor, b.phone, b.name, b.address from pet_test_usg a inner join pet_users c on a.userid = c.userid inner join pet_test_customer b on a.customerid = b.id where a.status < 3 and g.customerid in (". implode(', ', $c) .") and a.id not in (". implode(', ', $data->list) .") order by a.id asc";
...
//   $sql = "update pet_test_usg set customerid = $customerid, typeid = $data->typeid, cometime = $data->cometime, calltime = $data->calltime, status = 0, recall = $data->calltime, note = '$data->note', userid = $userid, utemp = 1, time = ". time() ." where id = $data->id";
//   $db->query($sql);
...
//       $sql = "insert into pet_test_usg (customerid, typeid, cometime, calltime, note, status, recall, userid, time, called) values($p[id], ". $type[$row[0]] .", $cometime, $calltime, '', 5, $calltime, ". $doctor[$row[1]] .", ". time() .", 0)";
//       $db->query($sql);
//     }
//   }
//   $result['list'] = gettemplist();
...
//   $sql = "select a.*, c.first_name as doctor from pet_test_usg a inner join pet_users c on a.userid = c.userid where a.status = 5 $xtra order by a.id desc";
//   $v = $db->all($sql);
//   $e = array();
//   $l = array();
//   $list = array(
...
//   foreach ($v as $row) {
//     if ($row['customerid']) {
//       $sql = "select a.name as petname, a.customerid, b.* from pet_test_pet a inner join pet_test_customer b on a.customerid = b.id where a.id = $row[customerid]";
//       $p = $db->fetch($sql);
...
//   $sql = "select a.*, b.name, b.phone, b.address, c.first_name as doctor from pet_test_usg a inner join pet_test_customer b on a.customerid = b.id inner join pet_users c on a.userid = c.userid where utemp = 1 and (time between $start and $end) $xtra order by a.id desc limit 50";
//   $v = $db->all($sql);
//   foreach ($v as $row) {
//     $list[1] []= array(
//       'id' => $row['id'],
