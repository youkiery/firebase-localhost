...
function updatephysical() {
  ...
  $time = time();
  $sql = "update pet_test_physical set customer = '$data->name', phone = '$data->phone', address = '$data->address', name = '$data->petname', weight = '$data->weight', age = '$data->age', gender = $data->gender, species = '$data->species', serial = '$data->serial', sampletype = '$data->sampletype', samplenumber = '$data->samplenumber', samplesymbol = '$data->samplesymbol', samplestatus = '$data->samplestatus', symptom = '$data->symptom' where id = $data->id";
  $db->query($sql);

  foreach ($data->target as $tid => $target) {
    $sql = "select * from pet_test_physical_data where pid = $data->id and tid = $tid";
    if (empty($d = $db->fetch($sql))) $sql = "insert into pet_test_physical_data (pid, tid, value) values ($data->id, $tid, '$target')";
    else $sql = "update pet_test_physical_data set value = $target where id = $d[id]";
    $db->query($sql);
  }
...
function getlist() {
  ...
  $sql = "select a.*, c.name as doctor from pet_test_physical a inner join pet_users c on a.doctor = c.userid where a.phone like '%$data->key%' or a.customer like '%$data->key%' order by id desc limit 10 offset 0";
  $query = $db->query($sql);
  $list = array();
  
  while ($row = $query->fetch_assoc()) {
    $sql = "select tid, value from pet_test_physical_data where pid = $row[id]";
